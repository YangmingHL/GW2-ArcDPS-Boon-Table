...
uint64_t BuffTracker::NormalizeEventTime(uint64_t event_time, uint64_t now) {
	constexpr uint64_t wrap = uint64_t{1} << 32;
	constexpr uint64_t half_wrap = wrap / 2;
	uint64_t candidate = (now & ~(wrap - 1)) | (event_time & (wrap - 1));
	if (candidate > now && candidate - now > half_wrap) candidate -= wrap;
	else if (now > candidate && now - candidate > half_wrap) candidate += wrap;
	return candidate;
}
...
void BuffTracker::Process(cbtevent* event, ag* source, ag* destination, uint64_t now) {
	...
	const uint64_t event_time = NormalizeEventTime(event->time, now);
	...
	case CBTS_BUFFAPPLY:
		if (IsSelf(destination, event->dst_agent)) Apply(*event, event_time);
		break;
	case CBTS_BUFFCHANGE:
		if (IsSelf(destination, event->dst_agent)) Change(*event, event_time);
		break;
	...
	case CBTS_BUFFACTIVE:
		if (IsSelf(source, event->src_agent)) ActivateStack(*event, event_time);
		break;
	...
	if (event->is_statechange == CBTS_COMBAT) {
		if (event->is_buffremove != CBTB_NONE && IsSelf(source, event->src_agent)) {
			...
			Apply(*event, event_time);
		}
	}
}

void BuffTracker::Apply(const cbtevent& event, uint64_t event_time) {
	...
	auto& stacks = effects_[effect_id].stacks;
	auto [stack_iterator, inserted] = stacks.try_emplace(stack_id);
	StackState& stack = stack_iterator->second;
	if (inserted) stack.activation_order = next_activation_order_++;
	stack.duration_ms = static_cast<uint32_t>(event.value);
	const EffectDefinition* definition = FindEffect(effect_id);
	stack.active = event.is_shields != 0
		|| (definition && definition->stacking == EffectStacking::Intensity)
		|| stacks.size() == 1;
	stack.end_time = stack.active ? event_time + stack.duration_ms : 0;
}

void BuffTracker::Change(const cbtevent& event, uint64_t event_time) {
	...
	stack.active = true;
	stack.end_time = event_time + duration;
}
...
void BuffTracker::ActivateStack(const cbtevent& event, uint64_t event_time) {
	...
	for (auto& state : effects_ | std::views::values) {
		if (const auto stack = state.stacks.find(stack_id); stack != state.stacks.end()) {
			...
			stack->second.end_time = event_time + stack->second.duration_ms;
			return;
		}
	}
}
...
std::optional<ActiveEffect> BuffTracker::Snapshot(uint32_t effect_id, EffectStacking stacking, uint64_t now) {
	...
	std::optional<uint64_t> next_start;
	for (;;) {
		for (auto it = stacks.begin(); it != stacks.end();) {
			if (it->second.active && it->second.end_time <= now) {
				if (stacking == EffectStacking::Duration) next_start = it->second.end_time;
				it = stacks.erase(it);
			} else {
				++it;
			}
		}
		if (!next_start || stacking != EffectStacking::Duration) break;

		auto next = std::ranges::min_element(stacks, {}, [](const auto& pair) {
			return pair.second.active ? UINT64_MAX : pair.second.activation_order;
		});
		if (next == stacks.end() || next->second.active) break;
		next->second.active = true;
		next->second.end_time = *next_start + next->second.duration_ms;
		next_start.reset();
	}
	if (stacks.empty()) {
